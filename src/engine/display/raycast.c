...
static t_vect	find_intersections(t_data *data, t_vect *dir, t_raycast *raycast)
{
	...
	while (42)
	{
		tmpdist = intersection_distances(tmp, dir, raycast, 0);
		tmp.x += (tmpdist * dir->x);
		tmp.y += (tmpdist * dir->y);
		hit_wall = check_if_wall(data, tmp);
		if (hit_wall == 2)
		{
			tmpdist = intersection_distances(tmp, dir, raycast, 1);
			tmp.x += (tmpdist * dir->x);
			tmp.y += (tmpdist * dir->y);
			hit_wall = check_if_wall(data, tmp);
			if (hit_wall == 2 && check_door(data, tmp, raycast->cardinal_direction))
			{
				raycast->isdoor = 1;
				break ;
			}
...
void	render_fov_view(t_data *data)
{
	...
	while (raycast.v_line_ct < SCREEN_WIDTH)
	{
		...
		raycast.distance *= (cos(get_angle(&data->player.vect, &raycast.ray_dir) / (180/M_PI)));//removing fisheye
